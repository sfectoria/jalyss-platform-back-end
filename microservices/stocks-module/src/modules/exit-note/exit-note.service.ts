  ...
  async create(createExitNoteDto: CreateExitNoteDto) {
    ...
    for (const line of lines) {
      ...
      if (stockArticle) {
        ...
        throw new Error(
          `L'article ${line.articleId} n'existe pas dans le stock ${createExitNoteDto.stockId}.`,
        );
      }
    }
      ...
      where['OR'] = [
        {
          salesInvoice: {
            some: {
              saleChannelId: { in: salesChannelsIds.map((elem) => +elem) },
            },
          },
        },
        {
          salesDeliveryInvoice: {
            some: {
              salesChannelsId: { in: salesChannelsIds.map((elem) => +elem) },
            },
          },
        },
        {
